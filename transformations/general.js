    ...
    'DirectiveLiteral|StringLiteral|NumericLiteral'(path) {
      ...
      try {
        if (path.node.extra.exoressionValue) {
          if (path.node.extra.expressionValue)
            path.node.value = path.node.extra.expressionValue;
        }
      } catch (e) {
        console.error(e);
      }
  ...
  traverse(ast, {
    'FunctionDeclaration|FunctionExpression'(path) {
      ...
      path.node.params.forEach((elem, i) => {
        ...
        else paramsToRename[elem.name] = `param${i}`;
      });

      for (const [key, value] of Object.entries(paramsToRename)) {
